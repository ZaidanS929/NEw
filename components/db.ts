...
import { articles, InsertArticle, InsertUser, users } from "../drizzle/schema";
...
export async function getAllArticles(publishedOnly: boolean = false) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  if (publishedOnly) {
    query = query.where(eq(articles.published, 1)) as typeof query;
  }
...
export async function getArticlesByCategory(category: string, publishedOnly: boolean = false) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  const result = await db.select().from(articles).where(and(...conditions)).orderBy(desc(articles.createdAt));
  return result;
}
